...
NETTLE_MINIMUM=3.4.1
    AC_ARG_VAR(NETTLE_CFLAGS, [C compiler flags for nettle])
    AC_ARG_VAR(NETTLE_LIBS, [linker flags for nettle])
    if test x$NETTLE_LIBS = x; then
	AC_CHECK_LIB(nettle, nettle_base64url_decode_init, [cryptolib="nettle"], [AC_MSG_ERROR([[
  ***
  *** Libnettle $NETTLE_MINIMUM was not found.
  ...
  if test x$NETTLE_LIBS != x; then
    ...
    AC_ARG_VAR(HOGWEED_CFLAGS, [C compiler flags for hogweed])
    AC_ARG_VAR(HOGWEED_LIBS, [linker flags for hogweed])
    if test x$HOGWEED_LIBS = x; then
	AC_CHECK_LIB(hogweed, nettle_ed25519_sha512_verify, [], [AC_MSG_ERROR([[
  *** 

...
void MVKViewportCommandEncoderState::encodeImpl(uint32_t stage) {
    ...
    if (_cmdEncoder->_pDeviceFeatures->multiViewport) {
		...
		for (uint32_t vpIdx = 0; vpIdx < vpCnt; vpIdx++) {
			...
			/* Hack for Cw Bug 18797: flipped viewports which had negative offset don't work on Intel Macs, FB9095683 */
			/* Adjust height so we don't take Y below 0. This will cause distortion, but it's still better than not having anything drawn. */
			if (mtlViewports[vpIdx].height < 0 && mtlViewports[vpIdx].originY + mtlViewports[vpIdx].height < 0) {
				VkPhysicalDeviceProperties properties;
				_cmdEncoder->getSubpass()->getRenderPass()->getPhysicalDevice()->getProperties(&properties);
				if (properties.vendorID == 0x1002) /* AMD */ {
					MVKLogInfo("Trimming flipped viewport height from %f to %f.", mtlViewports[vpIdx].height, -mtlViewports[vpIdx].originY);
					mtlViewports[vpIdx].height = -mtlViewports[vpIdx].originY;
				}
			}
		}
#if MVK_MACOS_OR_IOS

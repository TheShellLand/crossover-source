...
public:
  ...
  static Qualifiers removeCommonQualifiers(Qualifiers &L, Qualifiers &R) {
    ...
    if (L.hasAddressSpace() && R.hasAddressSpace() &&
        L.getAddressSpace() == R.getAddressSpace()) {
      Q.setAddressSpace(L.getAddressSpace());
      L.removeAddressSpace();
      R.removeAddressSpace();
    } else if (L.hasLatentAddressSpace() && R.hasLatentAddressSpace() &&
        L.getLatentAddressSpace() == R.getLatentAddressSpace()) {
      Q.setLatentAddressSpace(L.getLatentAddressSpace());
      L.removeLatentAddressSpace();
      R.removeLatentAddressSpace();
    }
    return Q;
  }
  ...
  // Latent address space and actual address space are mutually exclusive.
  // Furthermore, actual address space overrules latent.  So, setting an actual
  // address space clears the latent address space.  Attempting to set a latent
  // address space when there's an actual address space asserts.
  bool hasAddressSpace() const { return (Mask & AddressSpaceMask) && !(Mask & LatentAddressSpaceMask); }
  bool hasLatentAddressSpace() const { return Mask & LatentAddressSpaceMask; }
  LangAS getAddressSpace() const {
    if (Mask & LatentAddressSpaceMask) return LangAS::Default;
    return static_cast<LangAS>(Mask >> AddressSpaceShift);
  }
  LangAS getLatentAddressSpace() const {
    if (!(Mask & LatentAddressSpaceMask)) return LangAS::Default;
    return static_cast<LangAS>(Mask >> AddressSpaceShift);
  }
  ...
  void setAddressSpace(LangAS space) {
    ...
    Mask = (Mask & ~(AddressSpaceMask | LatentAddressSpaceMask))
  ...
  void setLatentAddressSpace(LangAS space) {
    assert((unsigned)space <= MaxAddressSpace);
    assert(!hasAddressSpace());
    Mask = (Mask & ~AddressSpaceMask)
         | (((uint32_t) space) << AddressSpaceShift) | LatentAddressSpaceMask;
  }
  void removeLatentAddressSpace() {
    if (hasLatentAddressSpace())
      setAddressSpace(LangAS::Default);
  }
  void addLatentAddressSpace(LangAS space) {
    setLatentAddressSpace(space);
  }
  ...
  bool hasNonLatentQualifiers() const
  {
    if (hasLatentAddressSpace())
      return Mask & ~(LatentAddressSpaceMask | AddressSpaceMask);
    return Mask & ~LatentAddressSpaceMask;
  }
  ...
  void addQualifiers(Qualifiers Q) {
    ...
    if (!(Q.Mask & ~CVRMask))
      ...
      if (Q.hasLatentAddressSpace() && !hasAddressSpace())
        addLatentAddressSpace(Q.getLatentAddressSpace());
      if (Q.hasObjCGCAttr())
  ...
  void removeQualifiers(Qualifiers Q) {
    ...
    if (!(Q.Mask & ~CVRMask))
      ...
      if (getObjCLifetime() == Q.getObjCLifetime())
        removeObjCLifetime();
      if (Q.hasLatentAddressSpace()) {
        if (hasLatentAddressSpace() && getLatentAddressSpace() == Q.getLatentAddressSpace())
          removeLatentAddressSpace();
      }
      else if (Q.hasAddressSpace() && hasAddressSpace() && getAddressSpace() == Q.getAddressSpace())
        removeAddressSpace();
    }
  }
  ...
  void addConsistentQualifiers(Qualifiers qs) {
    ...
    Mask |= qs.Mask & ~(AddressSpaceMask | LatentAddressSpaceMask);

    if (qs.hasLatentAddressSpace()) {
      if (!hasAddressSpace())
        setLatentAddressSpace(qs.getLatentAddressSpace());
    } else if (qs.hasAddressSpace()) {
      if (hasAddressSpace())
        assert(getAddressSpace() == qs.getAddressSpace());
      else
        setAddressSpace(qs.getAddressSpace());
    }
  }
...
private:
  ...
  //           |C R V|U|GCAttr|Lifetime|LAS|AddressSpace|
  uint32_t Mask = 0;
  ...
  static const uint32_t LatentAddressSpaceMask = 0x200;
  static const uint32_t LatentAddressSpaceShift = 9;
  static const uint32_t AddressSpaceMask =
      ~(CVRMask | UMask | GCAttrMask | LifetimeMask | LatentAddressSpaceMask);
...
public:
  ...
  /// Return the latent address space of this type.
  inline bool hasLatentAddressSpace() const;
  inline LangAS getLatentAddressSpace() const;
...
public:
  ...
  bool hasLatentAddressSpace() const { return Quals.hasLatentAddressSpace(); }
  LangAS getLatentAddressSpace() const { return Quals.getLatentAddressSpace(); }
...
public:
  ...
  public:
    ...
    bool covariesReturnType() const { return (Data & CovariesReturnType); }
    ExtParameterInfo withCovariesReturnType(bool Covaries) const {
      ExtParameterInfo copy = *this;
      if (Covaries)
        copy.Data |= CovariesReturnType;
      else
        copy.Data &= ~CovariesReturnType;
      return copy;
    }
...
inline bool QualType::hasNonLatentQualifiers() const {
  return getQualifiers().hasNonLatentQualifiers();
}
...
/// Return the latent address space of this type.
inline bool QualType::hasLatentAddressSpace() const {
  return getQualifiers().hasLatentAddressSpace();
}

/// Return the latent address space of this type.
inline LangAS QualType::getLatentAddressSpace() const {
  return getQualifiers().getLatentAddressSpace();
}

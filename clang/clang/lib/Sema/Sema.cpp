...
class SemaPPCallbacks : public PPCallbacks {
  ...
  void EnterSystemAS(SourceLocation Loc) {
  ...
  void ExitSystemAS(SourceLocation Loc) {
...
public:
  ...
  virtual void FileChanged(SourceLocation Loc, FileChangeReason Reason,
                           ...
                           SrcMgr::CharacteristicKind PrevFileType,
    ...
    case EnterFile: {
      ...
      if (S->LangOpts.Interop6432 && FileType != SrcMgr::C_LocalUser)
        EnterSystemAS(Loc);
      break;
    }
    case RenameFile:
      if (S->LangOpts.Interop6432) {
        bool EnterSystem = FileType != SrcMgr::C_LocalUser;
        bool ExitSystem = PrevFileType != SrcMgr::C_LocalUser;
        if (EnterSystem && !ExitSystem) EnterSystemAS(Loc);
        if (!EnterSystem && ExitSystem && SysCount > 0) ExitSystemAS(Loc);
      }
      break;
    case ExitFile:
      ...
      if (S->LangOpts.Interop6432 &&
          PrevFileType != SrcMgr::C_LocalUser &&
          SysCount > 0)
        ExitSystemAS(Loc);
      break;
    default:

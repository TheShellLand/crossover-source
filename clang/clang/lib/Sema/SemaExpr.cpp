...
ExprResult Sema::DefaultFunctionArrayConversion(Expr *E, bool Diagnose) {
  ...
  if (Ty->isFunctionType()) {
    ...
    E = ImpCastExprToType(E, Context.getPointerType(Ty, /*HonorASPragma=*/true),
...
ExprResult Sema::CallExprUnaryConversions(Expr *E) {
  ...
  // to function type.
  if (Ty->isFunctionType()) {
    Res = ImpCastExprToType(E, Context.getPointerType(Ty, /*HonorASPragma=*/true),
                            CK_FunctionToPointerDecay).get();
    if (Res.isInvalid())
      return ExprError();
  }
  ...
  {
    // Calls must be done on function pointers in the default address space.
    E = Res.get();
    Ty = E->getType();
    if (const PointerType *PointerTy = Ty->getAs<PointerType>())
    {
      ...
      if (PointeeTy->isFunctionType() && Quals.getAddressSpace() != LangAS::Default)
      {
        PointeeTy = Context.removeAddrSpaceQualType(PointeeTy);
        if (PointeeTy.getQualifiers().isAddressSpaceSupersetOf(Quals))
        {
          Ty = Context.getQualifiedType(Context.getPointerType(PointeeTy), Ty.getQualifiers());
          Res = ImpCastExprToType(E, Ty, CK_AddressSpaceConversion).get();
          if (Res.isInvalid())
            return ExprError();
        }
      }
...
ExprResult Sema::DefaultVariadicArgumentPromotion(Expr *E, VariadicCallType CT,
  ...
  if (AS != static_cast<LangAS>(-1))
  {
    QualType Ty = E->getType();
    if (const PointerType *PTy = Ty->getAs<PointerType>())
    {
      QualType PointeeTy = PTy->getPointeeType();
      Qualifiers PointeeQuals = PointeeTy.getQualifiers();
      LangAS PointeeAS = PointeeQuals.getAddressSpace();
      if (PointeeAS != AS)
      {
        PointeeTy = Context.removeAddrSpaceQualType(PointeeTy);
        PointeeTy = Context.getAddrSpaceQualType(PointeeTy, AS);
        Ty = Context.getQualifiedType(Context.getPointerType(PointeeTy), Ty.getQualifiers());

        Qualifiers DesiredQuals;
        DesiredQuals.setAddressSpace(AS);
        if (DesiredQuals.isAddressSpaceSupersetOf(PointeeQuals))
        {
          ExprResult ExprRes = ImpCastExprToType(E, Ty, CK_AddressSpaceConversion);
          if (ExprRes.isInvalid())
            return ExprError();
          E = ExprRes.get();
        }
        ...
        {
          Diag(E->getBeginLoc(), diag::err_typecheck_incompatible_address_space)
              << E->getType() << Ty << AA_Passing << E->getSourceRange();
        }
      }
    }
  }
...
recoverFromMSUnqualifiedLookup(Sema &S, ASTContext &Context,
  ...
  CXXScopeSpec SS;
  auto *NNS =
      NestedNameSpecifier::Create(Context, nullptr, true, RD->getQualTypeForDecl().getTypePtr());
...
Sema::CreateBuiltinArraySubscriptExpr(Expr *Base, SourceLocation LLoc,
  ...
  if (ResultType->isVoidType() && !getLangOpts().CPlusPlus) {
    ...
    if (!ResultType.hasNonLatentQualifiers()) VK = VK_RValue;
...
bool Sema::GatherArgumentsForCall(SourceLocation CallLoc, FunctionDecl *FDecl,
  ...
  if (CallType != VariadicDoesNotApply) {
    ...
    // Otherwise do argument promotion, (C99 6.5.2.2p7).
      ...
      if (LangOpts.Interop6432)
      {
        if (is32BitInteropCC(Proto->getCallConv()))
          AS = LangAS::ptr32;
        else
          AS = LangAS::Default;
      }
      for (Expr *A : Args.slice(ArgIx)) {
        ExprResult Arg = DefaultVariadicArgumentPromotion(A, CallType, FDecl, AS);
        Invalid |= Arg.isInvalid();
        AllArgs.push_back(Arg.get());
      }
    }
...
static FunctionDecl *rewriteBuiltinFunctionDecl(Sema *Sema, ASTContext &Context,
  ...
  for (QualType ParamType : FT->param_types()) {
    ...
    unsigned idx = i++;
    ExprResult ArgRes =
        Sema->DefaultFunctionArrayLvalueConversion(ArgExprs[idx]);
    if (ArgRes.isInvalid())
      return nullptr;
    Expr *Arg = ArgRes.get();
    ...
    if (FT->getExtParameterInfo(idx).covariesReturnType())
    {
      assert(Context.hasSameCVRUnqualType(ParamType->getPointeeType(),
                                          FT->getReturnType()->getPointeeType()) &&
             "Return type should match param which covaries it");
      assert(ReturnType == FT->getReturnType() &&
             "Can't have two params which covary return type");

      PointeeType = Context.getAddrSpaceQualType(ReturnType->getPointeeType(), AS);
      ReturnType = Context.getPointerType(PointeeType);
    }
  }
  ...
  QualType OverloadTy = Context.getFunctionType(ReturnType,
                                                OverloadParams, EPI,
                                                DeclType.getQualifiers());
...
Sema::BuildCompoundLiteralExpr(SourceLocation LParenLoc, TypeSourceInfo *TInfo,
  ...
  if (getLangOpts().Interop6432 &&
      literalType.getAddressSpace() == LangAS::Default) {
    literalType = Context.getAddrSpaceQualType(literalType, Context.StorageAddrSpace);
  }
...
Sema::ActOnCastExpr(Scope *S, SourceLocation LParenLoc,
                    ...
                    bool isTruncateCast, bool isASCast) {
...
static bool checkConditionalNullPointer(Sema &S, ExprResult &NullExpr,
  ...
  if ((!PointerTy->isAnyPointerType() && !PointerTy->isBlockPointerType()))
    return true;
  QualType NullTy = NullExpr.get()->getType();
  bool AnyAS = false;
  auto NullPteeTy = NullTy->getPointeeType();
  if (!NullPteeTy.isNull())
    AnyAS = NullPteeTy.getAddressSpace() ==
            PointerTy->getPointeeType().getAddressSpace();
  if (!NullExpr.get()->isNullPointerConstant(S.Context,
                                             Expr::NPC_ValueDependentIsNull,
                                             AnyAS))
    return true;
...
static QualType checkConditionalPointerCompatibility(Sema &S, ExprResult &LHS,
  ...
  QualType ResultTy = [&, ResultAddrSpace]() {
    if (ResultAddrSpace != LangAS::Default) {
...
checkConditionalObjectPointersCompatibility(Sema &S, ExprResult &LHS,
  ...
  if (((lhptee->isVoidType() && rhptee->isIncompleteOrObjectType()) ||
       (rhptee->isVoidType() && lhptee->isIncompleteOrObjectType())) &&
      (lhQual.hasAddressSpace() != rhQual.hasAddressSpace() ||
       lhQual.getAddressSpace() != rhQual.getAddressSpace()))
  {
    LangAS ResultAddrSpace = LangAS::Default;
    LangAS LAddrSpace = lhQual.getAddressSpace();
    LangAS RAddrSpace = rhQual.getAddressSpace();
    ...
    {
      lhQual.setAddressSpace(ResultAddrSpace);
      lhptee = S.Context.getQualifiedType(lhptee.getUnqualifiedType(), lhQual);
      LHSTy = S.Context.getPointerType(lhptee);
      LHS = S.ImpCastExprToType(LHS.get(), LHSTy, CK_AddressSpaceConversion);
    }
    if (RAddrSpace != ResultAddrSpace)
    {
      rhQual.setAddressSpace(ResultAddrSpace);
      rhptee = S.Context.getQualifiedType(rhptee.getUnqualifiedType(), rhQual);
      RHSTy = S.Context.getPointerType(rhptee);
      RHS = S.ImpCastExprToType(RHS.get(), RHSTy, CK_AddressSpaceConversion);
    }
  }
  // ignore qualifiers on void (C99 6.5.15p3, clause 6)
  if (lhptee->isVoidType() && rhptee->isIncompleteOrObjectType()) {
    // Figure out necessary qualifiers (C99 6.5.15p6)
    QualType destPointee = S.Context.getQualifiedType(lhptee, rhQual);
  ...
  if (rhptee->isVoidType() && lhptee->isIncompleteOrObjectType()) {
    QualType destPointee = S.Context.getQualifiedType(rhptee, lhQual);
    QualType destType = S.Context.getPointerType(destPointee);
    // Add qualifiers if necessary.
...
checkPointerTypesForAssignment(Sema &S, QualType LHSType, QualType RHSType) {
  ...
  bool incompatibleAddressSpaces = false;
  if (!lhq.compatiblyIncludes(rhq)) {
    // Treat address-space mismatches as fatal.  TODO: address subspaces
    if (!lhq.isAddressSpaceSupersetOf(rhq))
    {
      ConvTy = Sema::IncompatiblePointerDiscardsQualifiers;
      incompatibleAddressSpaces = true;
    }
...
checkBlockPointerTypesForAssignment(Sema &S, QualType LHSType,
  ...
  // Allow the left latent address space to override the right latent
  // (or absent) address space.
  if ((LQuals.hasLatentAddressSpace() &&
       (!RQuals.hasLatentAddressSpace() ||
        LQuals.getLatentAddressSpace() != RQuals.getLatentAddressSpace())) ||
       (!LQuals.hasLatentAddressSpace() && RQuals.hasLatentAddressSpace())) {
    RQuals.removeLatentAddressSpace();
    if (LQuals.hasLatentAddressSpace())
      RQuals.setLatentAddressSpace(LQuals.getLatentAddressSpace());
  }
  // Allow address space promotions, for e.g. 32on64 interop.
...
QualType Sema::CheckAddressOfOperand(ExprResult &OrigOp, SourceLocation OpLoc) {
  ...
  return Context.getPointerType(OpTy,
          /*HonorASPragma=*/OpTy->isFunctionType() &&
                            OpTy.getQualifiers().hasLatentAddressSpace());
}

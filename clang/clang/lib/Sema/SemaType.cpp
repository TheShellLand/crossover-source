...
QualType Sema::BuildObjCTypeParamType(const ObjCTypeParamDecl *Decl,
                                      ...
                                      bool FailOnError) {
  QualType Result = Decl->getQualTypeForDecl();
  if (!Protocols.empty()) {
    bool HasError;
    ...
    case DeclaratorContext::LambdaExprParameterContext:
      ...
      if (!SemaRef.getLangOpts().CPlusPlus14 ||
        ...
        T = SemaRef.ReplaceAutoType(
            T, CorrespondingTemplateParam->getQualTypeForDecl());
      }
      break;
...
static FileID getNullabilityCompletenessCheckFileID(Sema &S,
  ...
  if (fileInfo.getFileCharacteristic() != SrcMgr::C_User &&
      fileInfo.getFileCharacteristic() != SrcMgr::C_LocalUser &&
...
QualType Sema::BuildAddressSpaceAttr(QualType &T, Expr *AddrSpace,
  ...
  if (!AddrSpace->isValueDependent()) {
    ...
    unsigned addrSpaceValue = static_cast<unsigned>(addrSpace.getZExtValue());
    LangAS ASIdx = static_cast<LangAS>(-1);
    if (LangOpts.Interop6432)
    {
      for (unsigned i = 0; i < (unsigned)LangAS::FirstTargetAddressSpace; ++i)
        if (Context.getTargetAddressSpace(static_cast<LangAS>(i)) == addrSpaceValue)
          ASIdx = static_cast<LangAS>(i);
    }
    if (ASIdx == static_cast<LangAS>(-1))
      ASIdx = getLangASFromTargetAS(addrSpaceValue);
...
static void deduceInterop6432ImplicitAddrSpace(TypeProcessingState &State,
                                               QualType &T, TypeAttrLocation TAL) {
  Sema &S = State.getSema();
  ASTContext &Context = S.Context;
  Declarator &D = State.getDeclarator();
  ...
  // Don't apply latent address space to typedefs of Objective-C pointer types.
  if (T->isObjCObjectPointerType())
    return;
  ...
  bool IsFuncType =
      ...
      D.getTypeObject(ChunkIndex).Kind == DeclaratorChunk::Function;
  bool IsFunc =
      IsFuncType && D.isDeclarationOfFunction() && TAL == TAL_DeclName;

  if (DS.getStorageClassSpec() != DeclSpec::SCS_typedef &&
      !isa<RecordType>(T) && !IsFunc)
    return;

  if (// Do not deduce addr space for pointee type; that's taken care of by
      // the pointer address space.
      IsPointee ||
      // Do not deduce addr space for function return type and function type
      // that's not for a declaration, otherwise it will fail some sema check.
      IsFuncReturnType || (IsFuncType && !IsFunc) ||
      // Do not deduce addr space for member types of struct.
  ...
  LangAS AS = IsFunc ? Context.StorageAddrSpace : Context.DefaultAddrSpace;
  Qualifiers Quals = T.getQualifiers();
  if (Quals.hasLatentAddressSpace() && Quals.getLatentAddressSpace() == AS)
    return;

  QualType NewT = Context.getLatentAddrSpaceQualType(T, AS);
  if (!NewT.isNull())
    T = NewT;
}
...
static void processTypeAttrs(TypeProcessingState &state, QualType &type,
  ...
  if (state.getSema().getLangOpts().Interop6432 &&
      !type.getQualifiers().hasAddressSpace())
    deduceInterop6432ImplicitAddrSpace(state, type, TAL);
}

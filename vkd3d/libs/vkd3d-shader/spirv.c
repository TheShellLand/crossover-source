...
struct vkd3d_dxbc_compiler
{
    ...
    bool decorate_invariant;
    bool uses_precise;
    uint32_t vertex_output_position_id;
        ...
        struct vkd3d_shader_message_context *message_context)
{
    ...
    for (i = 0; i < compile_info->option_count; ++i)
    {
        ...
        switch (option->name)
        {
            ...
            case VKD3D_SHADER_COMPILE_OPTION_HACK_DECORATE_INVARIANT:
                compiler->decorate_invariant = option->value;
                break;
        ...
        const struct vkd3d_shader_dst_param *dst, enum vkd3d_shader_input_sysval_semantic sysval)
{
    ...
    if (builtin && builtin->spirv_builtin == SpvBuiltInPosition && compiler->shader_type == VKD3D_SHADER_TYPE_VERTEX)
        compiler->vertex_output_position_id = id;
        ...
        const struct vkd3d_shader_instruction *instruction)
{
    ...
    if (instruction->handler_idx == VKD3DSIH_IEQ)
    {
        if (src[0].reg.data_type != VKD3D_DATA_UINT)
        {
            type_id = vkd3d_spirv_get_type_id(builder, VKD3D_SHADER_COMPONENT_UINT, component_count);
            src0_id = vkd3d_spirv_build_op_bitcast(builder, type_id, src0_id);
        }
        if (src[1].reg.data_type != VKD3D_DATA_UINT)
        {
            type_id = vkd3d_spirv_get_type_id(builder, VKD3D_SHADER_COMPONENT_UINT, component_count);
            src0_id = vkd3d_spirv_build_op_bitcast(builder, type_id, src0_id);
        }
        ...
        const struct vkd3d_shader_compile_info *compile_info, struct vkd3d_shader_code *spirv)
{
    ...
    /* Invariant hack CW Bug 18486 */
    if (compiler->decorate_invariant && compiler->vertex_output_position_id && compiler->uses_precise)
        vkd3d_spirv_build_op_decorate(builder, compiler->vertex_output_position_id, SpvDecorationInvariant, NULL, 0);

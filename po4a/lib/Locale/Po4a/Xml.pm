...
CodeWeavers Addition: Escape quotes in strings to produce Android-compatible xml.
...
sub treat_content {
	...
	if (!$self->{options}{'nostrip'}) {
		...
		# Remove escaping on quotes if we are going to re-escape
		if($self->{options}{'escapequotes'})
		{
			for my $text (@paragraph) {
				$text =~ s/\\'/'/g;
				$text =~ s/\\"/"/g;
			}
		}
